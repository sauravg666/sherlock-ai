...
import { registerSherlockModelCommand } from "./research-tools/sherlock-ai-model.js";
import { installSherlockHeader } from "./research-tools/header.js";
...
export default function researchTools(pi: ExtensionAPI): void {
	...
	pi.on("session_start", async (_event, ctx) => {
		await installSherlockHeader(pi, ctx, cache);
	});

	registerAlphaTools(pi);
	registerDiscoveryCommands(pi);
	registerSherlockModelCommand(pi);
