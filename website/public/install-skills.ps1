...
function Resolve-VersionMetadata {
  ...
  return [PSCustomObject]@{
    ...
    DownloadUrl = if ($env:SHERLOCK_AI_INSTALL_SKILLS_ARCHIVE_URL) { $env:SHERLOCK_AI_INSTALL_SKILLS_ARCHIVE_URL } else { "https://github.com/sauravg666/sherlock-ai/archive/refs/tags/v$resolvedVersion.zip" }
  }
}
...
function Resolve-InstallDir {
  ...
  if ($ResolvedScope -eq "Repo") {
    return Join-Path (Get-Location) ".agents\skills\sherlock-ai"
  }
  if ($ResolvedScope -eq "OpenCode") {
    return Join-Path (Get-Location) ".opencode\skills\sherlock-ai"
  }

  $codexHome = if ($env:CODEX_HOME) { $env:CODEX_HOME } else { Join-Path $HOME ".codex" }
  return Join-Path $codexHome "skills\sherlock-ai"
}
...
$tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) ("sherlock-ai-skills-install-" + [System.Guid]::NewGuid().ToString("N"))
New-Item -ItemType Directory -Path $tmpDir | Out-Null

try {
  $archivePath = Join-Path $tmpDir "sherlock-ai-skills.zip"
  $extractDir = Join-Path $tmpDir "extract"

  Write-Host "==> Downloading Sherlock skills $resolvedVersion"
  ...
  if (-not $sourceRoot) {
    throw "Could not find extracted Sherlock archive."
  }
  ...
  Write-Host "Sherlock skills $resolvedVersion installed successfully."
} finally {
  if (Test-Path $tmpDir) {
